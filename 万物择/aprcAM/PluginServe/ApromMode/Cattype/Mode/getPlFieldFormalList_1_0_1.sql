-- ##Title 插件端-web后台-审批模式-通用配置-供需需求信息管理-发布逻辑-查询已发布的字段内容不是上传文件或供方填写的字段名称列表
-- ##Author 卢文彪
...
from (select t1.guid                        as plateFieldGuid
           , t2.operation
           , case
                 when (t2.content_source = '3') then (select content_source
                                                      from coz_model_am_aprom_plate_field_settings_formal
                                                      where plate_field_guid = t1.guid
                                                        and cat_tree_code = 'demand'
                                                        and del_flag = '0')
                 else t2.content_source end as contentSource
           , t3.code                        as plateFieldContentCode
           , t2.cat_tree_code               as catTreeCode
           , CONCAT(
            '{ChildRows_aprcAM\\PluginServe\\ApromMode\\Cattype\\Mode\\getPlFieldContFormalList_1_0_1:plateFieldGuid=''',
            t1.guid, '''}')                 as `fields`
