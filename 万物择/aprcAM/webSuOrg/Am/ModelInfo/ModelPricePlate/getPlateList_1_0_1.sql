-- ##Title  web机构-审批模式-切换合作项目-需求范围管理-需求范围设置-型号模式-创建型号产品介绍-获取板块列表
-- ##Author 卢文彪
-- ##CreateTime 2023-07-27
-- ##Describe 查询：复合接口，嵌套结构，仅需返回2层
-- ##Describe 表名：coz_model_am_aprom_plate_formal
-- ##Describe 排序：板块的顺序升序
-- ##CallType[QueryData]
...
-- ##output plateCode string[6] 板块名称code;板块名称code
...
-- ##output fields.relateFlag enum[0,1] 1;字段名称是否被关联：0-否，1-是。前端：仅显示未被关联的字段名称列表，已被关联的字段名称根据字段内容选中触发动态展示
...
-- ##output fields.fieldBizGuid string[36] 字段业务随机guid;字段业务随机guid
-- ##output fields.plateFieldCode string[20] 字段名称code(字段名称是固化时有值);字段名称code(字段名称是固化时有值)

select
t.guid as plateGuid
,t.alias as plateAlias
,fd.code as plateCode 
,t.norder as plateNorder
,CONCAT('{ChildRows_aprcAM\\webSuOrg\\Am\\ModelInfo\\ModelPricePlate\\getPlateFieldList_1_0_1:plateGuid=''',t.guid,'''}') as `fields`
from
{url:[http://127.0.0.1:8011/html?SqlCmdName=aprcAM\Web\Suprice\Cattype\Price\getTableNamePrefixByBizType_1_0_1&bizType={bizType}&DBC=w_a]/url}_plate_formal t 
inner join coz_model_fixed_data fd on t.fixed_data_code = fd.guid
where 
