-- ##Title web-对接专员操作管理-购方/供方对接管理-购方用户采购管理/供应渠道供应管理-审批模式-查看详情(供方供应完成)
-- ##Author 卢文彪
-- ##CreateTime 2023-09-13
-- ##Describe 查询 目标所属类型的品类采购交易状态下的供方供应完成信息,t1.业务guid=t2.guid
-- ##Describe 其他过滤条件:t3.支付状态是支付成功
-- ##Describe 表名：coz_server3_cate_dealstatus_statistic t1,coz_order t2,coz_demand_request_supply t3,coz_demand_request t4
-- ##CallType[QueryData]
...
-- ##output operationTime319 char[19] 2023-12-12 12:12:12;供方办理通知日期
-- ##output operationTime320 char[19] 2023-12-12 12:12:12;供方供应完成日期
...
select left(t2.supply_done_time, 19)        as operationTime320
     ,left(t6.create_time, 19)        as operationTime319
     , t5.category_name               as categoryName
     , t5.cattype_name                as cattypeName
     , t4.user_name                   as orgName
...
from coz_server3_cate_dealstatus_statistic t1
         ...
         inner join coz_demand_request_price t3 on t2.request_price_guid = t3.guid
         inner join coz_demand_request_supply t4 on t3.request_supply_guid = t4.guid
         inner join coz_demand_request t5 on t2.request_guid = t5.guid
         inner join coz_order_am_notice t6 on t2.guid = t6.order_guid
         left join coz_order_cancel t7 on t2.guid = t7.order_guid
where t1.del_flag = '0'
  and t1.dstatus = '319'
  and t2.del_flag = '0'
  and t3.del_flag = '0'
