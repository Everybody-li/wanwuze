-- ##Title web-对接专员操作管理-购方/供方对接管理-购方用户采购管理/供应渠道供应管理-交易模式-退款退货订单-查看交易有关人员
-- ##Author 卢文彪
...
-- ##input bizGuid char[36] NOTNULL;业务guid:退款退货订单-订单仲裁guid
...
# t1的业务guid记录的是订单取消guid或订单退货guid
select
    appu.user_name                    as demandUserName
...
from
    ...
    inner join coz_order_cancel           cocancel on cojudge.biz_guid = cocancel.guid
    inner join coz_order                  corder on cocancel.order_guid = corder.guid
    ...
    inner join coz_org_info               coi on t1.supply_user_id = coi.user_id and coi.user_id <> ''
...
where
  ...
  and cojudge.result in ('1', '2')
  and cocancel.refund_pay_status in ('2', '3')
union all
select
    appu.user_name                    as demandUserName
  , concat('(+86)', appu.phonenumber) as demandUserPhone
...
from
    coz_server3_cate_dealstatus_statistic t1
    inner join coz_order_judge            cojudge on t1.biz_guid = cojudge.guid
    inner join coz_order_refund           corefund on cojudge.biz_guid = corefund.guid
    inner join coz_order                  corder on corefund.order_guid = corder.guid
    inner join sys_app_user               appu on t1.demand_user_id = appu.guid
    inner join coz_cattype_sd_path        cdp on t1.sd_path_guid = cdp.guid
    inner join coz_lgcode_fixed_data      lfd on cdp.lgcode_guid = lfd.guid
    left join coz_org_info               coi on t1.supply_user_id = coi.user_id and coi.user_id <> ''
    left join  sys_user                   dsu on t1.demand_sys_user_guid = dsu.user_id
    left join  sys_user                   ssu on t1.supply_sys_user_guid = ssu.user_id
where
      t1.biz_guid = '{bizGuid}'
  ...
  and corder.pay_status = '2'
  and cojudge.result in ('1', '2')
  and corefund.confirm_refund_pay_flag = '1';
