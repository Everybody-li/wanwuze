-- ##Title web-对接专员操作管理-购方/供方对接管理-购方用户采购管理/供应渠道供应管理-审批模式-查看详情(退款退货订单)
-- ##Author 卢文彪
-- ##CreateTime 2023-09-13
-- ##Describe 查询 目标所属类型的品类采购交易状态下的退款退货订单信息,t1.业务guid=t2.guid
-- ##Describe 其他过滤条件:t2.退款成功的
-- ##Describe 表名：coz_server3_cate_dealstatus_statistic t1,coz_order_cancel t2,coz_order t3,coz_demand_request_supply t4,coz_demand_request t5,coz_demand_request_price t6
-- ##CallType[QueryData]
...
-- ##output operationTime321 char[19] 2023-12-12 12:12:12;退款退货到账日期
...
select left(t2.refund_pay_time, 19)  as operationTime321
     , t4.category_name               as categoryName
     , t4.cattype_name                as cattypeName
     , t6.user_name                   as orgName
...
from coz_server3_cate_dealstatus_statistic t1
         ...
         inner join coz_demand_request t4 on t3.request_guid = t4.guid
         inner join coz_demand_request_price t5 on t3.request_price_guid = t5.guid
         inner join coz_demand_request_supply t6 on t5.request_supply_guid = t6.guid
where t1.del_flag = '0'
  and t2.del_flag = '0'
  and t3.del_flag = '0'
  and t4.del_flag = '0'
  ...
  and (t2.refund_pay_status = '2' or t2.refund_pay_status = '3')
  and t1.sd_path_guid = '{sdPathGuid}'
  and (t4.category_name like '%{categoryName}%' or '{categoryName}' = '')
  and ({file[aprcAM/Web/DjRoleUser/DeCatDealStatusStatistic/getStatusComSerachCondi_1_0_1.sql]/file})
order by t2.refund_pay_time desc
