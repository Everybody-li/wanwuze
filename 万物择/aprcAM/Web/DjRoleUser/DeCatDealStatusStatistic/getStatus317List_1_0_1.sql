-- ##Title web-对接专员操作管理-购方/供方对接管理-购方用户采购管理/供应渠道供应管理-审批模式-查看详情(需方采购支付)
-- ##Author 卢文彪
-- ##CreateTime 2023-09-11
-- ##Describe 查询 目标所属类型的品类采购交易状态下的需方采购支付信息,t1.业务guid=t4.guid
-- ##Describe 其他过滤条件:t4.支付状态是支付成功
-- ##Describe 表名：coz_server3_cate_dealstatus_statistic t1,coz_demand_request t2,coz_demand_request_supply t3,coz_order t4
-- ##CallType[QueryData]
...
-- ##output operationTime317 char[19] 2023-12-12 12:12:12;需方采购支付日期
...
select left(t4.price_time, 19)        as operationTime317
     , left(t2.create_time, 19)       as operationTime313
...
from coz_server3_cate_dealstatus_statistic t1
         ...
         left join coz_order_cancel t6 on t2.guid = t6.order_guid
where t1.del_flag = '0'
  and t2.del_flag = '0'
  and t3.del_flag = '0'
  and t4.del_flag = '0'
