-- ##Title web-对接专员操作管理-购方/供方对接管理-购方用户采购管理/供应渠道供应管理-审批模式-退款退货订单-查看交易有关人员
-- ##Author 卢文彪
...
-- ##input bizGuid char[36] NOTNULL;业务guid:退款退货订单-订单取消guid(供方取消订单导致退款)
...
from coz_server3_cate_dealstatus_statistic t1
             inner join coz_order_cancel cocancel on t1.biz_guid = cocancel.guid
...
where t1.biz_guid = '{bizGuid}'
  and t1.del_flag = '0'
  and t1.dstatus = '321'
  and (cocancel.refund_pay_status = '2' or cocancel.refund_pay_status = '3')
