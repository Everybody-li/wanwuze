-- ##Title web后台-审批模式-通用配置/配置管理-供应报价信息管理-信息格式排序-采购/供应需求信息-修改板块排序
-- ##Author 卢文彪
...
-- ##Describe 表名：表名前缀_plate
-- ##CallType[ExSql]

-- ##input bizType enum[1,2,3] NOTNULL;业务菜单类型：1-审批模式下的通用或非通用的供应报价信息管理，2-审批报价的供应报价信息配置，3-审批报价的供应报价信
...
set @flag1=case when((select norder from {url:[http://127.0.0.1:8011/html?SqlCmdName=aprcAM\Web\Suprice\Cattype\Price\getTableNamePrefixByBizType_1_0_1&bizType={bizType}&DBC=w_a]/url}_plate where guid='{plateGuid}')={oldNOrder}) then 1 else 0 end
;
...
,update_by='{curUserId}'
,update_time=now()
where norder<={newNOrder} and norder>={oldNOrder} and biz_guid='{bizGuid}' and @norderflag>=0 and guid<>'{plateGuid}' and @flag1=1 and del_flag='0'
;
update {url:[http://127.0.0.1:8011/html?SqlCmdName=aprcAM\Web\Suprice\Cattype\Price\getTableNamePrefixByBizType_1_0_1&bizType={bizType}&DBC=w_a]/url}_plate
set norder=norder+1
,update_by='{curUserId}'
