-- ##Title 插件端-web后台-审批模式-通用配置-供应报价信息管理-发布逻辑-克隆品类(发布）
-- ##Author 卢文彪
...
set @publishFlag = (select case
                               when exists(
                                       select 1 from coz_category_am_suprice_log where cattype_guid = '{cattypeGuid}')
                                   then '1'
                               else '0' end)
;
...
where category_guid = '{categoryGuid}'
  and @publishFlag = '1'
;
insert into coz_model_am_suprice_plate
( guid
, cattype_guid
, category_guid
, biz_guid
...
select t2.new_t_guid
     ...
     , '{curUserId}'
     , now()
from coz_model_am_suprice_plate_formal t1
         inner join
...
where t1.del_flag = '0'
  ...
  and t2.biz_type = '2'
  and @publishFlag = '1'
  and not exists(select 1 from coz_model_am_suprice_plate where guid = t2.new_t_guid)
;
...
where t1.del_flag = '0'
  ...
  and t2.biz_type = '2'
  and @publishFlag = '1'
  and not exists(select 1 from coz_model_am_suprice_plate_formal where guid = t2.new_t_guid)
;
insert into coz_model_am_suprice_plate_field
( guid
, cattype_guid
, category_guid
, biz_guid
...
where t1.category_guid = '{cattypeGuid}'
  ...
  and t1.del_flag = '0'
  and @publishFlag = '1'
  and not exists(select 1 from coz_model_am_suprice_plate_field where guid = t2.new_t_guid)
;
...
where t1.category_guid = '{cattypeGuid}'
  ...
  and t1.del_flag = '0'
  and @publishFlag = '1'
  and not exists(select 1 from coz_model_am_suprice_plate_field_formal where guid = t2.new_t_guid)
;
insert into coz_model_am_suprice_plate_field_content
( guid
, cattype_guid
, category_guid
, biz_guid
...
select t2.new_t_guid
     ...
     , '{curUserId}'
     , now()
from coz_model_am_suprice_plate_field_content_formal t1
         inner join coz_model_am_clone_guid t2 on t1.guid = t2.old_t_guid
...
where t1.category_guid = '{cattypeGuid}'
  ...
  and t1.del_flag = '0'
  and @publishFlag = '1'
  and not exists(select 1 from coz_model_am_suprice_plate_field_content where guid = t2.new_t_guid)
;
...
where t1.category_guid = '{cattypeGuid}'
  ...
  and t1.del_flag = '0'
  and @publishFlag = '1'
  and not exists(select 1 from coz_model_am_suprice_plate_field_content_formal where guid = t2.new_t_guid)
;
