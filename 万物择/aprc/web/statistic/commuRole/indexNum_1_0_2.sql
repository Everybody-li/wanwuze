...
select 
ifnull((select count(1) from coz_target_object t1 where t1.del_flag='0'),0) as objectNum
,ifnull((select count(1) from coz_target_object t1 where t1.del_flag='0' and not exists(select 1 from coz_serve2_commu_task_tobject t8 where t8.object_guid=t1.guid and t8.del_flag='0' and t8.result='1')),0) as canAssignUserNum
