...
-- ##input hasRes string[1] NOTNULL;是否有进行沟通结果操作(0-否，1-是)，必填
...
,t1.result
,case when(t1.result=''0'') then ''0'' else ''1'' end resultFlag
,t2.name as nickName
...
where 
(t2.phonenumber like''%{phonenumber}%'' or t3.org_name like''%{phonenumber}%'' or ''{phonenumber}''='''') and t1.del_flag=''0'' and t3.del_flag=''0'' and (t1.commu_task_guid=''{commuTaskGuid}'' or ''{commuTaskGuid}''='''')
)t
where
t.resultFlag=''{hasRes}''
order by t.id2 desc,t.id3 desc
limit ?,?;
';
