...
,t.id
from
coz_org_gyv2_relate_staff_log t
inner join
coz_org_info t1
on t1.user_id=t.org_user_id
where t.del_flag=''0'' and t.staff_type=''2'' and t.detach_flag=''0'' and t.staff_user_id=''{curUserId}'' and (t1.name like ''%{orgName}%'' or ''{orgName}''='''')
union all
select
t1.name as orgName
,left(t1.create_time,16) as registerTime
...
coz_org_info t1
on t1.user_id=t.org_user_id
where t.del_flag=''0'' and t.staff_type=''2'' and t.detach_flag=''1'' and t.staff_user_id=''{gyv2UserId}'' and (t1.name like ''%{orgName}%'' or ''{orgName}''='''')
)t
order by t.id desc
limit ?,?;
';
