...
;
update coz_category_deal_deadline t
set day='{day}'
,update_by='{curUserId}'
,update_time=now()
where category_guid='{categoryGuid}' or (cattype_guid='{categoryGuid}' and not exists(select 1 from coz_category_deal_deadline_log where deadline_guid=t.guid))