...
from
coz_category_info
where not exists(select 1 from coz_model_plate_field where cat_tree_code='{catTreeCode}' and category_guid='{categoryGuid}' and biz_type='{bizType}' and name='{plateFieldName}' and del_flag='0') and guid='{categoryGuid}'
union all
select
UUID() as guid
