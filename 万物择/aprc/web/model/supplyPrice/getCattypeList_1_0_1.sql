-- ##Title web-品类类型查询供应报价信息列表
-- ##Author 卢文彪
-- ##CreateTime 2020-06-05
-- ##Describe web-品类类型查询供应报价信息列表
-- ##CallType[QueryData]
...
,t1.norder
,case when (not exists (select 1 from coz_model_plate where biz_type='2' and category_guid=t1.guid and del_flag='0')) then '0' when (exists (select 1 from coz_model_plate where biz_type='2' and category_guid=t1.guid and publish_flag='0')) then '0' else '1' end as flag1
,case when (not exists (select 1 from coz_model_plate_field where biz_type='2' and category_guid=t1.guid and del_flag='0')) then '0' when (exists (select 1 from coz_model_plate_field where biz_type='2' and category_guid=t1.guid and publish_flag='0')) then '0' else '1' end as flag2
,case when ((exists(select 1 from coz_model_plate_field where biz_type='2' and category_guid=t1.guid and del_flag='0' and (operation='1' or operation='2'))) and (not exists (select 1 from coz_model_plate_field_content where biz_type='2' and category_guid=t1.guid and del_flag='0'))) then '0' when (exists (select 1 from coz_model_plate_field_content where biz_type='2' and category_guid=t1.guid and publish_flag='0')) then '0' else '1' end as flag3
,left(t.publish_time,19) as publishTimes
from
coz_cattype_fixed_data t1
inner join
coz_category_supply_price t
