-- ##Title web-查询交易信息跟踪管理列表交易类-已上架_1_0_3
-- ##Author 卢文彪
-- ##CreateTime 2020-06-05
-- ##Describe web-查询交易信息跟踪管理列表交易类-已上架_1_0_3
-- ##CallType[QueryData]
...
,case when (exists(select 1 from {url:[http://127.0.0.1:8011/html?SqlCmdName=aprcAM\Com\Utils\Model\getModeTableNamePrefixByCattype_1_0_1&cattypeGuid={cattypeGuid}&DBC=w_a]/url}_log where category_guid=t1.guid and del_flag=''0'' and t1.cattype_guid=''{cattypeGuid}'')) then ''是'' else ''否'' end as dealModePF
,case when (exists(select 1 from {url:[http://127.0.0.1:8011/html?SqlCmdName=aprcAM\Com\Utils\Model\getSPriceTableNamePrefixByCattype_1_0_1&cattypeGuid={cattypeGuid}&DBC=w_a]/url}_log where category_guid=t1.guid and del_flag=''0'' and t1.cattype_guid=''{cattypeGuid}'')) then ''是'' else ''否'' end as dealPricePF
, case
