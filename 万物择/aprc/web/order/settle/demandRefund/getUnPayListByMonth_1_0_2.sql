...
where 
left(t.create_time,7)=''{month}'' and t.del_flag=0 and (t1.order_no like ''%{orderNo}%'' or ''{orderNo}''='''') and (t.supply_done_flag=''0'') and t4.result<>''0''
union all
select
t4.guid as judgeGuid
,t4.result as judgeResult
...
on t1.demand_user_id=t3.guid
where 
left(t.create_time,7)=''{month}'' and t.del_flag=0 and (t1.order_no like ''%{orderNo}%'' or ''{orderNo}''='''') and t.supply_accept<>''1'' and t4.result<>''0''
)t
