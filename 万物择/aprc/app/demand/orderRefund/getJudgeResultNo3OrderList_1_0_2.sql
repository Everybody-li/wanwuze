...
,concat(''【'',t4.name,''】\n采购编号:【'',t3.order_no,''】\n本订单退货办理，系统自动办理完成。默认供方验收通过。点击【我知道了】后在【订单状态管理】了解进度。'') as autoSubmitRefundProvMsg
from 
coz_order_refund t1
left join 
coz_order_judge t2
...
where 
t3.demand_user_id=''{curUserId}'' and (t2.result=''1'' or t2.result=''2''or t2.result=''4'') and t3.parent_guid='''' and t1.prove_logistic_img='''' and t1.prove_supply_sign_date is null and t3.sd_path_guid=''{sdPathGuid}''
union all
select 
t2.guid as judgeGuid
,t2.result as judgeResult
...
on t3.category_guid=t4.guid 
where 
t3.demand_user_id=''{curUserId}'' and (t2.result=''1'' or t2.result=''2''or t2.result=''4'') and t3.parent_guid='''' and t1.demand_done_flag=''0'' and t3.sd_path_guid=''{sdPathGuid}''
)t
