alter table coz_model_plate_field_content_formal
    add column norder int default 1 null comment '顺序' after content;
...
select display as `key`
     , t.*
from (
         select ifnull(ifnull(t1.value, (select name from coz_model_fixed_data where guid = t3.content)),
                       t3.content)                                              as display
              , t3.plate_field_formal_guid                                      as fieldGuid
              , t.name                                                          as fieldname
              , t3.category_guid
              , t3.biz_type
              , (select code from coz_model_fixed_data where guid = t3.content) as contentFDCode
              , t3.id
              , t.content_source
              , t3.cat_tree_code
              , t3.norder
         from coz_model_plate_field_content_formal t3
                  left join
