-- ##Title app-招聘方式管理-应聘人员查找-查询应聘人员-邀约面试
-- 使用方:WebAPP
-- ##Author 卢文彪
-- ##CreateTime 2023-09-14
-- ##Describe app-招聘方式管理-应聘人员查找-查询应聘人员-邀约面试
...
-- ##Describe 接口类型：中间件
-- ##Describe 表名：
-- ##Describe 沟通模式-应聘需求供方(招聘方)招聘信息表：coz_chat_supply_request_demand t1
-- ##Describe 应聘需求供方(招聘方)招聘信息详情表：coz_chat_supply_request_demand_detail t2
-- ##Describe 应聘招聘用户在线沟通好友申请表：coz_chat_friend_apply t3 
-- ##Describe 沟通模式-招聘信息内容详情表：coz_chat_employ_detail   t4 
-- ##CallType[ExSql]

-- ##input deRequestGuid char[36] NOTNULL;应聘需求guid，必填
-- ##input recruitGuid char[36] NOTNULL;应聘信息guid，必填
-- ##input reimg string[50] NOTNULL;应聘方简介图片，必填
-- ##input userId char[36] NOTNULL;应聘方用户id，必填
-- ##input userName string[50] NOTNULL;应聘方用户姓名，必填
-- ##input userPhone string[11] NOTNULL;应聘方用户姓名，必填
...
;
insert into coz_chat_supply_request_demand_detail(guid,request_supply_guid,fd_code,fd_name,fd_value_guid,fd_value,status,del_flag,create_by,create_time,update_by,update_time)
select
uuid()
...
;
insert into coz_chat_supply_request_demand(guid,de_request_guid,recruit_guid,recruit_user_id,recruit_user_name,recruit_user_phone,recruit_reimg,recommend_type,del_flag,create_by,create_time,update_by,update_time)
select
...
where not exists(select 1 from coz_chat_supply_request_demand where de_request_guid='{deRequestGuid}' and recruit_guid='{recruitGuid}' and recommend_type='{recType}' and del_flag='0')
limit 1
;
