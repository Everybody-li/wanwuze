-- ##Title app-应聘方-根据末级场景和字节内容查询品类列表
-- ##Author 卢文彪
-- ##CreateTime 2020-06-05
-- ##Describe app-应聘方-根据末级场景和字节内容查询品类列表
-- ##CallType[QueryData]
...
(
	...
	,case when (
	(exists(select 1 from coz_app_user_permission_detail where biz_guid=t.guid and user_id=''{curUserId}'' and type=4 and del_flag=0)) or (exists(select 1 from coz_app_user_permission where user_id=''{curUserId}'' and type=2 and del_flag=0 and status=1))
