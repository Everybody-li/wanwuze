...
set @flag5=(select case when exists(select 1 from coz_target_object_org where object_guid=(select guid from coz_target_object where phonenumber='{phonenumber}'and del_flag='0' limit 1) and org_name='{orgName}' and del_flag='0') then '0' else '1' end)
;
...
from
coz_target_object
where phonenumber='{phonenumber}' and del_flag='0' and ('{orgName}'<>'' or '{orgType}'<>'' or '{roleType}'<>'' or '{registerCityCode}'<>'' or '{registerCity}'<>'') and @flag5='1'
;
