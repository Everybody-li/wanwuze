...
-- ##input cloneFlag enum[0,1] NOTNULL;品类类型的克隆禁用标志：0-禁用，1-启用
...
# 审批模式-供需需求信息表
INSERT INTO coz_category_am_aprom_mode
(category_guid,cattype_guid,create_by,create_time) 
select
'{categoryGuid}' as categoryGuid
,'{cattypeGuid}' as cattype_guid
,'{curUserId}' as create_by
...
# 审批模式-供应报价信息表
INSERT INTO coz_category_am_suprice
(biz_guid,category_guid,cattype_guid,create_by,create_time) 
select
'{categoryGuid}' as bizGuid
,'{categoryGuid}' as categoryGuid
,'{cattypeGuid}' as cattype_guid
,'{curUserId}' as create_by
,now() as create_time
...
where '{mode}' in ('2','3')  and category_guid='{cattypeGuid}' and del_flag='0' and day>0
order by id desc
limit 1
;

# 交易模式和审批模式 品类验收期限信息日志表
...
,'{priceMode}'
,'{serveFeeFlag}'
,if({cloneFlag} = 0 || ''='{publishTime}',null,now()) 
,if({cloneFlag} = 0 || ''='{publishTime}',0,'2') 
,'{curUserId}'
,now() 
,'{curUserId}'
,now()
