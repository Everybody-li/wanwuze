...
,(select count(1) from coz_serve_order_kpi where supply_user_id=''{orgUserId}'' and category_guid=t3.guid and del_flag=''0'') as orderNum
from
coz_org_info t1
inner join
...
where 
t1.user_id=''{orgUserId}'' and t1.del_flag=''0'' and (t3.name like ''%{categoryName}%'' or ''{categoryName}''='''')
order by t2.id desc
limit ?,?;
';
