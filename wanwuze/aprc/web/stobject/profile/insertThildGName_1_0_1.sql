...
set @flag1=(select case when exists(select 1 from coz_target_object_profile where third_dynamic_gcode='{fixedDatacode}' and profile_template_guid='{profileTemplateGuid}' and object_guid='{objectGuid}' and del_flag='0') then '0' else '1' end)
;
insert into coz_target_object_profile(guid,object_guid,profile_template_guid,type,third_dynamic_gcode,del_flag,create_by,create_time,update_by,update_time)
select
uuid() as guid
