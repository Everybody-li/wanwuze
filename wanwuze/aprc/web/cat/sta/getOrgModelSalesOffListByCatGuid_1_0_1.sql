-- ##Title 运营经理操作系统-品类交易管理-品类供应渠道管理-审批模式-有供应渠道-查看品类型号详情-型号未上架数量-(供方型号)列表
-- ##Author 卢文彪
...
,t1.id
,case when (exists(select 1 from coz_category_supplier_am_model_plate where del_flag='0' and model_guid=t3.guid and status='1')) then '1' else '0' end as editModelFlag
,case when (exists(select 1 from coz_category_supplier_am_model_price_plate where del_flag='0' and model_guid=t3.guid and status='1')) then '1' else '0' end as editModelPriceFlag
,case when (exists(select 1 from coz_category_am_modelprice_log where del_flag='0' and biz_guid=t3.guid)) then '1' else '0' end as publishPriceFlag
from
coz_org_info t1
inner join
