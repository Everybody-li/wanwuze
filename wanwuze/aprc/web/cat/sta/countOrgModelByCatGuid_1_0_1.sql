-- ##Title 运营经理操作系统-品类交易管理-品类供应渠道管理-审批模式-有供应渠道-查看品类型号详情-表头上方选项卡数量
-- ##Author 卢文彪
-- ##CreateTime 2020-06-05
-- ##Describe 运营经理操作系统-品类交易管理-品类供应渠道管理-审批模式-有供应渠道-查看品类型号详情-表头上方选项卡数量
...
select
sum(saleOnModelNum) as saleOnModelNum
,sum(modelNum)-sum(saleOnModelNum) as saleOffModelNum
from
(
select
(select count(1) from coz_category_supplier_am_model a where a.supplier_guid=t2.guid and a.del_flag='0') as modelNum
,(select count(1) from coz_category_supplier_am_model a where a.supplier_guid=t2.guid and a.del_flag='0'
