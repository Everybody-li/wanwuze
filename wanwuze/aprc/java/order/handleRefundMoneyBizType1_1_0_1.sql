...
-- ##Describe 订单取消的退款业务处理
-- ##CallType[ExSql]

-- ##input orderCancelGuid char[36] NOTNULL;订单取消guid,必填
...
set refund_pay_status='{refundPayFlag}'
  , refund_pay_time=now() {dynamic:refundFee[,refund_fee={refundFee}]/dynamic}
...
from coz_order t1
         inner join coz_order_cancel t2 on t1.guid = t2.order_guid
         inner join coz_demand_request t3 on t1.request_guid = t3.guid
         left join coz_order_judge t4 on t2.guid = t4.biz_guid
where t2.guid = '{orderCancelGuid}';
...
where biz_guid = @judgeGuid
  and @catMode = 2
;
# 审批模式:更新品类交易状态为供方取消订单-退款到账,审批模式无仲裁
update coz_server3_cate_dealstatus_statistic
set update_time         = now()
  , dstatus             = 321
  , demand_sys_user_guid=@demandSysUserGuid
  , supply_sys_user_guid=@supplySysUserGuid
where biz_guid = '{orderCancelGuid}'
  and @catMode = 3
;
