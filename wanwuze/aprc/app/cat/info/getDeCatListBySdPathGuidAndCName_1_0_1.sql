...
select * from coz_org_info
where phonenumber like '%187%';
    ...
    (
        ...
        from
            (
                select
                  ...
                  , case
                        when (exists(select 1
                                     ...
                                     where user_id = '{curUserId}' and type = 2 and del_flag = '0' and status = '1'))
                            then '1'
                        else '0' end as flag1
                  , case
                        when (exists(select 1
                                     from
                                         coz_app_user_permission_detail
                                     where
                                           biz_guid = t.guid
                  ...
                  , case
                        when (exists(select 1
                                     from
                                         coz_category_deal_rule_log        a
                                         right join (
                                                        select category_guid, max(id) as MID
                                                        from coz_category_deal_rule_log
                                                        group by category_guid
                                                    )                      b on a.id = b.MID
                                         left join  coz_category_deal_rule c on a.deal_rule_guid = c.guid
                                     where
                                           a.category_guid = t.guid
                                       and c.serve_fee_flag = '1'
                                       and a.del_flag = '0'
                                       and c.del_flag = '0') and
                              exists(select 1 from coz_category_deal_mode_log where category_guid = t.guid) and
                              exists(select 1 from coz_category_supply_price_log where category_guid = t.guid) and
                ...
                where
#                       t2.sd_path_guid = '{sdPathGuid}'
                      t2.sd_path_guid = 'de7f5cd2-1f1e-11ed-afde-00163e2ca549'
                  and t.del_flag = '0'
#                   and (t.name like '%{categoryName}%' or '{categoryName}' = '')
                  and (t.name like '%汽车%' or '汽车' = '')
            ) t
    ) t
group by
    categoryGuid, categoryName, alias, img, categoryMode, buttonStatus, buttonStatusName, sdFlag, priceMode, sdPathGuid
