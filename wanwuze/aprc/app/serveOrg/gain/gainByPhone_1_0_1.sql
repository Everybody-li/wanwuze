...
,case when(@flag1='0') then '与当前服务机构已解绑，不可领取服务对象，请先加入服务机构！' when(@flag2='0') then '当前服务机构没有授权品类，不可领取' when(@flag3='0') then '该手机号在服务机构的当前所有品类类型下已被领取过了，不可领取' when(@flag6='0') then '自己无需领取自己' else '操作成功' end as msg