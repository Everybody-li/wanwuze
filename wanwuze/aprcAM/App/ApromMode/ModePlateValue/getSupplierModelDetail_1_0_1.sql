...
select
t3.name as supplyCompanyName
,t.guid as modelGuid
,t.name as modelName
,(select price_way from coz_category_am_modelprice_log where biz_guid=t.guid and del_flag='0' order by id desc limit 1) as priceWay
,(select concat('ARPC/WEB/AM/MODELPRICE/QRCODE/',left(qrcode,1),'/',qrcode) from coz_category_am_modelprice_log where biz_guid=t.guid and del_flag='0' order by id desc limit 1) as qrCode
,(select qrcode_url from coz_category_am_modelprice_log where biz_guid=t.guid and del_flag='0' order by id desc limit 1) as qrcodeUrl
,'识别没有成功，用微信扫一扫试一下' as invalidQrcodeMsg
,CONCAT('{ChildRows_aprcAM\\App\\ApromMode\\ModePlateValue\\getSupplierModelPlates_1_0_1:modelGuid=''',t.guid,'''}') as `plates`
from
coz_category_supplier_am_model t
    inner join coz_category_supplier t2 on t.supplier_guid = t2.guid
    inner join coz_org_info t3 on t2.user_id = t3.user_id
where
t.guid='{modelGuid}' and t.del_flag='0' and t2.del_flag ='0' and t3.del_flag='0'