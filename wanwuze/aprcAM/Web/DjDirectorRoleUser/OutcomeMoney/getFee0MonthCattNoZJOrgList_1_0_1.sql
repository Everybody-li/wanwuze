-- ##Title web-服务主管操作管理-服务主管业绩管理-购方/供方对接业绩管理-服务免费-非资金资源需求-产品费用月度详情-产品费用机构详情-查询列表
-- ##Author 卢文彪
-- ##CreateTime 2023-12-20
-- ##Describe 查询当前服务主管的业绩列表:某一月份下,查询归属服务主管的服务专员的有订单验收通过的供方机构列表,求和corder的产品费用(supply_fee),按订单供方和corder的验收通过时间的年份和月度分组求和,按机构创建时间倒序
-- ##Describe 查询条件:t1表中品类状态是验收通过,含交易模式和审批模式,t1的需方/供方对接专员是入参对接专员guid,t2的服务主管guid是入参目标用户id,cdr的收取服务费是免费,品类类型是非资金资源需求
-- ##Describe 表名：coz_server3_cate_dealstatus_statistic t1,coz_server3_cate_dealstatus_statistic_outcome t2,coz_order corder,coz_demand_request cdr,coz_demand_request_price cdrp,coz_cattype_fixed_data cfd,coz_org_info org
-- ##CallType[QueryData]
...
-- ##input orgName string[50] NULL;机构名称或手机号(模糊搜索)
