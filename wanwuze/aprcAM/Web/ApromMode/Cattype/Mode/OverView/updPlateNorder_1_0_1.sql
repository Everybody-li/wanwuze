-- ##Title web后台-审批模式-通用配置-供需需求信息管理-信息格式排序-采购/供应需求信息-修改板块排序
-- ##Author 卢文彪
...
-- ##Describe 表名：coz_model_am_aprom_plate
-- ##CallType[ExSql]
...
set @flag1=case when((select norder from coz_model_am_aprom_plate where guid='{plateGuid}')={oldNOrder}) then 1 else 0 end
;
...
,update_by='{curUserId}'
,update_time=now()
where norder<={newNOrder} and norder>={oldNOrder} and category_guid='{categoryGuid}' and @norderflag>=0 and guid<>'{plateGuid}' and @flag1=1 and del_flag='0'
;
update coz_model_am_aprom_plate
set norder=norder+1
,update_by='{curUserId}'
