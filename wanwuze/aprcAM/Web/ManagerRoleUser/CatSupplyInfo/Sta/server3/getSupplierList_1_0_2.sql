...
-- ##input targetUserId char[36] NULL;对接专员用户id,从对接(服务)专员点击进入时必传
...
-- ##output bindTime char[19] 关联日期;关联日期从对接(服务)专员点击进入时有值

select *
from (select t2.user_id               as orgUserId
           , t2.org_ID                as orgID
      ...
      from coz_category_supplier t1
           ...
           on t3.lgcode_guid = t4.guid
               left join coz_server3_sys_user_dj_bind sud on t2.user_id = sud.binded_user_id and sud.user_type = 2
      where t1.del_flag = '0'
        and t2.del_flag = '0'
        and (t2.name like '%{orgName}%' or '{orgName}' = '') {dynamic:targetUserId[and sud.binded_user_id ='{targetUserId}']/dynamic}) t
