-- ##Title web-对接专员操作管理-购方/供方对接管理-购方用户采购管理/供应渠道供应管理-审批模式-查看详情(供应取消订单)
-- ##Author 卢文彪
-- ##CreateTime 2023-09-11
-- ##Describe 查询 目标所属类型的品类采购交易状态下的供应取消订单信息,t1.业务guid=t2.guid
-- ##Describe 其他过滤条件:t3.支付状态是支付成功
-- ##Describe 表名：coz_server3_cate_dealstatus_statistic t1,coz_order_cancel t2,coz_order t3,coz_demand_request_supply t4,coz_demand_request t5
-- ##CallType[QueryData]
...
-- ##output operationTime318 char[19] 2023-12-12 12:12:12;供应取消订单日期
...
select left(t5.create_time, 19)       as operationTime318
...
from coz_server3_cate_dealstatus_statistic t1
         inner join
     coz_order_cancel t5
     on t1.biz_guid = t5.guid
         inner join
     coz_order t4
     on t4.guid = t5.order_guid
         inner join
     coz_demand_request t2
     on t2.guid = t4.request_guid
         inner join
