-- ##Title web-对接专员操作管理-购方/供方对接管理-购方用户采购管理/供应渠道供应管理-审批模式-查看详情(供方供应报价)
-- ##Author 卢文彪
-- ##CreateTime 2023-09-11
-- ##Describe 查询 目标所属类型的品类采购交易状态下的供方供应报价信息,t1.业务guid=t3.guid
-- ##Describe 其他过滤条件:t2.需求未取消未完成交易,t3.报价状态是已报价
...
-- ##output operationTime316 char[19] 2023-12-12 12:12:12;供方供应报价日期
...
select left(t4.price_time, 19)        as operationTime316
     , left(t3.create_time,19) as operationTime313
...
where t1.del_flag = '0'
  ...
  and t3.del_flag = '0'
  and t1.dstatus = '316'
and (t1.demand_user_id='{demandOrSupplyUserId}' or t1.supply_user_id='{demandOrSupplyUserId}' or '{demandOrSupplyUserId}'='')
