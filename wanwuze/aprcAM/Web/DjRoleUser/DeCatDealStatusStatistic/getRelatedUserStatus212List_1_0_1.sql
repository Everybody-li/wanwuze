-- ##Title web-对接专员操作管理-购方/供方对接管理-购方用户采购管理/供应渠道供应管理-交易模式-供方供应报价-查看交易有关人员
-- ##Author 卢文彪
...
-- ##input bizGuid char[36] NOTNULL;业务guid:供方供应报价-需求guid(需求与供方是一对多关系)
...
from
    ...
    inner join coz_demand_request                cdr on t1.biz_guid = cdr.guid
    inner join coz_demand_request_supply         drs on cdr.guid = drs.request_guid
    inner join coz_category_supplier             ccs on drs.supplier_guid = ccs.guid
    ...
    inner join coz_org_info                      coi on ccs.user_id = coi.user_id and coi.user_id <> ''
    inner join coz_demand_request_supply_server3 drsser3 on drs.guid = drsser3.request_supply_guid
    left join  sys_user                          dsu on t1.demand_sys_user_guid = dsu.user_id
    left join  sys_user                          ssu on drsser3.sys_user_guid = ssu.user_id
where
      t1.biz_guid = '{bizGuid}'
  and t1.del_flag = '0'
  and cdr.cancel_flag = '0'
  and cdr.done_flag = '0'
