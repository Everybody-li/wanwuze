-- ##Title web-对接专员操作管理-购方/供方对接管理-购方用户采购管理/供应渠道供应管理-交易模式-查看详情(供方取消订单)
-- ##Author 卢文彪
-- ##CreateTime 2023-09-13
-- ##Describe 查询 目标所属类型的品类采购交易状态下的供方取消订单信息,t1.业务guid=t4.guid
-- ##Describe 其他过滤条件:t2.取消对象是供方取消,t4.仲裁结果未裁决
-- ##Describe 表名：coz_server3_cate_dealstatus_statistic t1,coz_order_cancel t2,coz_order_judge t4,coz_order t5,coz_demand_request t6
-- ##CallType[QueryData]
...
-- ##output operationTime215 char[19] 2023-12-12 12:12:12;供应取消订单日期
...
select left(t5.create_time, 19)       as operationTime215
...
from coz_server3_cate_dealstatus_statistic t1
         inner join
     coz_order_judge t4
     on t1.biz_guid = t4.biz_guid
         inner join
     coz_order t3
     on t3.guid = t4.order_guid
         inner join
     coz_demand_request t2
     on t3.request_guid = t2.guid
         inner join
     coz_order_cancel t5
     on t5.guid = t4.biz_guid
         left join
