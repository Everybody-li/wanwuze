-- ##Title web-对接专员操作管理-购方/供方对接管理-购方用户采购管理/供应渠道供应管理-交易模式-需方删除需求-查看交易有关人员
-- ##Author 卢文彪
...
-- ##input bizGuid char[36] NOTNULL;业务guid:需方删除需求-需求guid
...
from
    ...
    left join  coz_org_info                      coi on cs.user_id = coi.user_id and coi.user_id <> ''
    left join  sys_user                          dsu on t1.demand_sys_user_guid = dsu.user_id
    left join  sys_user                          ssu on drss3.sys_user_guid = ssu.user_id
where
      t1.biz_guid = '{bizGuid}'
  and t1.del_flag = '0'
  and t1.dstatus = 213
