-- ##Title web后台-审批报价配置管理-xx供应路径-供应审批报价管理-品类审批报价管理-查询可克隆的型号列表
-- ##Author 卢文彪
...
select t3.cattype_guid                                                as cattypeGuid
     ...
     , t4.qrcode
     , case
           when (t4.biz_guid is null) then '非二维码'
           when (t4.price_way = '1') then '非二维码'
           else '二维码' end                                          as priceWayStr
     , case
           when (t4.publish_flag = '0' or t4.publish_flag is null) then '1'
           else '0' end                                               as publishBtnHighLightFlag
